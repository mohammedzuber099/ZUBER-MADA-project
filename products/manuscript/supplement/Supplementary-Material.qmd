...
│
├── data/
│   ├── raw-data/           # Contains the original input datasets (raw files)
│   └── processed-data/     # Contains the cleaned and transformed datasets
│
├── code/
...
## LASSO Cox Model Performance at 12 Months
To assess the discriminatory ability of the LASSO Cox regression model, we evaluated model performance at 12 months post-diagnosis. The linear predictor (risk score) was used to generate a receiver operating characteristic (ROC) curve, and area under the curve (AUC) was calculated. Additionally, we computed a confusion matrix using a default threshold of 0 for classification.

@tbl-lasso-auc. Model discrimination at 12 months, measured by area under the ROC curve (AUC). The LASSO Cox model achieved a moderate discriminative ability with an AUC of 0.664.
```{r}
#| label: tbl-lasso-auc
#| tbl-cap: "AUC for the LASSO Cox model at 12-month survival prediction."
#| echo: FALSE
paste("AUC at 12 months:", round(auc_val, 3))

```

@tbl-lasso-cm. Confusion matrix at 12 months using a classification threshold of 0. The model achieved high sensitivity (87.8%) but low specificity (25.7%), indicating it was better at correctly identifying deaths than survivals at this time point.
```{r}
#| label: tbl-lasso-cm
#| tbl-cap: "Confusion matrix for LASSO Cox model predicting 12-month mortality."
#| echo: FALSE
conf_matrix
